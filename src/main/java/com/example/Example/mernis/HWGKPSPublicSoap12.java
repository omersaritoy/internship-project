...
public class HWGKPSPublicSoap12
{
    ...
    private HWGConnectionProvider connectionProvider=new HWGHttpConnectionProvider();

    public HWGKPSPublicSoap12()
    {
    }

    public HWGKPSPublicSoap12(String url)
    {
        this.url=url;
    }

    public HWGKPSPublicSoap12 (String url,HWGConnectionProvider connectionProvider) {
        this.url=url;
    ...
    protected HWGRequestResultHandler createRequestResultHandler()
    {
        HWGRequestResultHandler handler = new HWGRequestResultHandler(HWGSoapVersion.v1_2);
        return handler;
    }
    
    private org.w3c.dom.Document createTCKimlikNoDogrulaRequest(final Long TCKimlikNo,final String Ad,final String Soyad,final Integer DogumYili,HWGRequestResultHandler __handler) throws Exception
    {
    ...
    {
        HWGRequestResultHandler __handler =createRequestResultHandler();
        ...
        org.w3c.dom.Node __result=HWGHelper.getResult(__handler.getOutputBody(), "TCKimlikNoDogrulaResult",false);
        if(__result!=null)
        {
            return HWGHelper.toBoolFromString(((org.w3c.dom.Element)__result).getTextContent());
        }
        return null;
    }
    protected void sendRequest(String soapAction,org.w3c.dom.Document soapBody, HWGRequestResultHandler handler) throws Exception
    {
        ...
        String __requestBody=HWGHelper.getStringFromDocument(soapBody);
        if(enableLogging)
        {
            System.out.println("requestDump: "+__requestBody);
        }
        HWGResponseData response=connectionProvider.sendRequest(url,__requestBody, __headers,handler);
        if(enableLogging)
        {
